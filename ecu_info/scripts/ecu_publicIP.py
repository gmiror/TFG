...
def ecu_pip():
    pub = rospy.Publisher('ecu_publicIP', String, queue_size=10) #Declares that the node is publishing to the chatter topic using the message type String.
	...
	print('\n')
        time.sleep(5)
        ecu_public_ip = '%s' % (ECU_Public_IP)
        rospy.loginfo(ecu_public_ip) #Performs triple-duty: the messages get printed to screen, it gets written to the Node's log file, and it gets written to rosout.
        pub.publish(ecu_public_ip) #Publishes a string to our chatter topic.
