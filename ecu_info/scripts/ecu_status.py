...
ECU_Status = cat(""+HOME_DIR+"/ECU_data/Status.txt")

def status():
    pub = rospy.Publisher('ecu_status', String, queue_size=10) #Declares that the node is publishing to the chatter topic using the message type String.
    ...
    while not rospy.is_shutdown(): #Check if your program exists
        time.sleep(5)
        ecu_status = '%s' % (ECU_Status)
        rospy.loginfo(ecu_status) #Performs triple-duty: the messages get printed to screen, it gets written to the Node's log file, and it gets written to rosout.
        pub.publish(ecu_status) #Publishes a string to our chatter topic.
